...
export function PlayingCardComponent({ card, isAnimating, delay = 0 }: PlayingCardComponentProps) {
  ...
  return (
    ...
    >
      <div className="w-full h-full p-1.5 flex flex-col justify-between">
        ...
        {/* Bottom Right */}
        <div
          className={cn("flex flex-col items-end leading-none rotate-180", isRed ? "text-red-600" : "text-gray-900")}
        >
          <span className="text-sm font-bold">{card.rank}</span>
          <span className="text-xs">{suitSymbol}</span>
        </div>
