...
export function PlinkoBoard({
  ...
  // Slight center bias (requested): small "spring" toward center + slightly biased peg bounce.
  // Set to 0 to disable.
  const centerBiasStrength = 0.00075 * scale;
  ...
  useEffect(() => {
    // keep reference balls in sync without dropping in-flight state
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      slots.forEach((slot, index) => {
        ...
        if ((ctx as any).roundRect) {
          ...
          roundRect(slot.x - slotWidth / 2, slot.y, slotWidth, slotHeight, slotRadius);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        // Smaller multiplier text (requested)
        ctx.font = `bold ${11 * scale}px sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        // Match original formatting: just render the raw number.
    ...
    const updatePhysics = () => {
      ...
      for (let i = 0; i < currentBalls.length; i++) {
        ...
        ball.vy += gravity;
        ball.vx *= friction;
        ball.vx += (startX - ball.x) * centerBiasStrength;
        ball.x += ball.vx;
        ball.y += ball.vy;
        ...
        for (const peg of pegs) {
          ...
          if (dist < minDist && dist > 0) {
            ...
            if (relVel < 0) {
              ...
              const toCenter = startX - ball.x;
              const biasDir = toCenter === 0 ? 0 : Math.sign(toCenter);
              const biasKick = biasDir * (0.18 + Math.random() * 0.32);
              const noiseKick = (Math.random() > 0.5 ? 1 : -1) * (0.06 + Math.random() * 0.18);
              ball.vx += biasKick + noiseKick;
            }
          }
        }

        // Cap horizontal velocity spikes to keep motion natural
        const maxVx = 8 * scale;
